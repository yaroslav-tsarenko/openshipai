...
app.post('/sign-up-carrier-account', async (req, res) => {
    ...
    try {
        const existingCarrier = await Carrier.findOne({ carrierAccountAccountEmail: formData.carrierAccountAccountEmail });
        if (existingCarrier) {
        ...
        const emailSubject = 'Congratulations!⭐ Your carrier account has been successfully created!';
        const emailText = `Dear ${formData.carrierAccountName},\n\nYour carrier account has been successfully created.\n\nThank you for registering with us.\n\nBest regards,\nOpenShipAI Team`;
        await sendEmail(formData.carrierAccountAccountEmail, emailSubject, emailText);
    ...
    try {
        const account = await stripe.accounts.create({
            ...
            individual: {
                ...
                last_name: 'Tsarenko',
                email: email,
                phone: '+1 971 353 8229', // Use a valid US phone number
                address: {
                    line1: '123 Main Street',
                    city: 'San Francisco',
                    state: 'CA',
                    postal_code: '94105',
                    country: 'US' // Ensure the address country matches the account country
                },
                dob: {
                    day: 1,
                    month: 1,
                ...
                ssn_last_4: '4323' // Provide the last four digits of SSN
            },
            business_profile: {
                url: 'https://openshipai.com' // Provide a business website
            },
            external_account: {
                object: 'bank_account',
                country: 'US',
                currency: 'usd',
                account_holder_name: 'Yaroslav Tsarenko',
                account_holder_type: 'individual',
                routing_number: '125000105', // Replace with a real routing number
                account_number: '168404318359' // Replace with a real account number
            },
            requested_capabilities: ['card_payments', 'transfers'], // Request necessary capabilities
            tos_acceptance: {
...
app.put('/update-load-status/:loadCredentialID', async (req, res) => {
...
app.get('/get-load-bids-by-load/:loadCredentialID', async (req, res) => {
    const { loadCredentialID } = req.params;
    try {
        const loadBids = await LoadBid.find({ loadCredentialID: loadCredentialID });
...
app.get('/driver-assigned-loads/:driverID', async (req, res) => {
    ...
    try {
        ...
        const assignedLoads = await Load.find({ loadAssignedDriverID: driverID });
        res.status(200).json(assignedLoads);
        console.log(assignedLoads)
    } catch (error) {
...
app.get('/get-load-bids-by-carrier/:userID', async (req, res) => {
    const { userID } = req.params;

    try {
        const loadBids = await LoadBid.find({ loadBidCarrierID: userID });
...
app.post('/google-login', async (req, res) => {
    ...
    try {
        let user = await Shipper.findOne({ userShipperEmail: email });
        if (user) {
            return res.status(200).send({ id: user.userShipperID, role: 'shipper' });
        }
        user = await Carrier.findOne({ carrierAccountAccountEmail: email });
        if (user) {
            return res.status(200).send({ id: user.carrierID, role: 'carrier' });
        }
        user = await Driver.findOne({ driverEmail: email });
        if (user) {
            return res.status(200).send({ id: user.driverID, role: 'driver' });
        }
...
app.put('/update-driver-location/:driverID/:driverLat/:driverLng', async (req, res) => {
    const { driverID, driverLat, driverLng } = req.params;
    ...
    try {
        ...
        driver.driverLat = parseFloat(driverLat);
        driver.driverLng = parseFloat(driverLng);
        await driver.save();
        console.log("Driver location updated to:", driverLat, driverLng);
...
app.put('/assign-load/:driverID', async (req, res) => {
    ...
    try {
        ...
        await driver.save();
        sendEmail(driver.driverEmail, 'Carrier assigned load!🚚', 'You have been assigned a new load. Check your dashboard for more details.');
...
app.post('/send-email-2', async (req, res) => {
    const { userType, letter, userID } = req.body;
    ...
    try {
        ...
        const email = user.carrierAccountAccountEmail;
        const subject = 'Hurry up! Shipper applied your bid!💰';
        const text = "You've received a new bid from a shipper. Log in to your account to view the details.";
        await sendEmail(email, subject, text);
        res.status(200).json({ message: 'Email sent successfully' });
    } catch (error) {
        console.error('Error sending email:', error);
...
app.post('/create-checkout-session-2', async (req, res) => {
    ...
    try {
        const session = await stripe.checkout.sessions.create({
            ...
            cancel_url: `http://localhost:3000/customer-deal-chat-conversation/${shipperID}`,
        });
